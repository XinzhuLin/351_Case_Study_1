    ...
    if (~isempty(hp))
        for column = 1:size(final_music,2)
            for index = 1:length(hp)
                ...
                final_impulse_response_individual(length(lp)+index,:) = gain_hp{index}(1,1).*lsim(transfer_function_list(end),original_music(:,column),time_vector); 
    ...
    if (~isempty(lp_hp))
        for column = 1:size(final_music,2)
            for index = 1:length(lp_hp)
                ...
                final_impulse_response_individual(length(lp)+length(hp)+index,:) = gain_lp_hp{index}(1,1).*lsim(transfer_function_list(end),original_music(:,column),time_vector); 
    ...
    if (~isempty(rlc_elements))
        for column = 1:size(final_music,2)
            for index = 1:length(rlc_elements)
                ...
                final_impulse_response_individual(length(lp)+length(hp)+length(lp_hp)+index,:) = gain_rlc{index}.*lsim(transfer_function_list(end),original_music(:,column),time_vector);
